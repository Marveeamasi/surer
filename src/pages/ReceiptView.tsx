...
const STATUS_CONFIG: Record<
  string,
  { label: string; bgClass: string; icon: any }
> = {
  pending: {
  ...
  settling: {
    ...
    bgClass: "bg-blue-400/20 text-blue-300",
    icon: Loader2,
  },
  pending_bank_details: {
    label: "Action Required",
    bgClass: "bg-orange-400/20 text-orange-300",
    icon: AlertTriangle,
  },
  dispute: {
    label: "In Dispute",
...
const ReceiptView = () => {
  ...
  const handlePayNow = () =>
    requirePin(
      "Confirm Payment",
      "Enter your PIN to proceed with payment.",
      executePayNow,
    );
  ...
  const executeDelete = async () => {
    ...
    if (error) {
      toast.error("Cannot delete this receipt");
      return;
    }
  ...
  const handleDelete = () =>
    requirePin(
      "Confirm Deletion",
      "Enter your PIN to permanently delete this receipt.",
      executeDelete,
    );
  ...
  const handleRetrySett = () =>
    requirePin(
      "Retry Settlement",
      "Enter your PIN to retry settlement.",
      executeRetrySett,
    );
  ...
  const needsForm = (type: string) =>
    ["release_specific", "refund", "reject"].includes(type);
  ...
  const startDecision = (type: string) => {
    ...
    if (needsForm(type)) setShowDecisionForm(true);
      ...
      requirePin("Confirm Decision", "Enter your PIN to confirm.", () =>
        submitDecision(type, "", ""),
      );
  };
  ...
  const submitDecision = async (
    ...
    try {
      ...
      if (isSender) {
        ...
        updateData.sender_decision_amount =
          type === "release_specific" ? parseFloat(amount) : null;
      } else {
      ...
      const newReceiverDec =
        updateData.receiver_decision === null && isSender
          ? null
          : isSender
      ...
      if (shouldRelease) {
        const releaseDecision =
          newSenderDec === "release_all"
            ? "release_all"
            : newSenderDec === "release_specific"
              ? "release_specific"
              : "refund";
        const releaseAmount =
          releaseDecision === "release_specific"
            ? isSender
              ? parseFloat(amount)
              : receipt.sender_decision_amount
            : null;
        ...
        try {
          ...
          if (releaseError || !releaseResult?.success) {
            ...
            if (releaseResult?.requiresBankDetails) {
              ...
              toast.error(
                `${errMsg} Go to Settings → Bank Details to fix this, then come back and tap Retry Settlement.`,
                { duration: 8000 },
              );
            } else {
              toast.error(errMsg);
      ...
      // Email — fire-and-forget via email.ts fallback chain (EmailJS → Nodemailer → Resend)
      const emailTarget = isSender ? receipt.receiver_email : user?.email || "";
      if (emailTarget) {
        const emailOpts =
          newStatus === "dispute"
            ? buildDisputeEmail(receipt.description, receipt.amount, receipt.id)
            : shouldRelease
              ? buildCompletedEmail(
                  receipt.description,
                  receipt.amount,
      ...
      toast.success(
        shouldRelease
          ? "✅ Settlement sent. Funds will arrive in bank accounts shortly."
          : newStatus === "dispute"
            ? "⚠️ Dispute started. You have 4 days to reach agreement."
            : "Decision recorded.",
      );
    ...
    return (
      <div className="space-y-3">
        ...
        <Button
          ...
          size="lg"
          className="w-full"
          onClick={() => startDecision("release_all")}
        >
        ...
        <Button
          ...
          size="lg"
          className="w-full"
          onClick={() => startDecision("release_specific")}
        >
        ...
        <Button
          ...
          size="lg"
          className="w-full"
          onClick={() => startDecision("refund")}
        >
          <XCircle className="w-5 h-5" /> Request Full Refund
        </Button>
        <p className="text-xs text-center text-muted-foreground">
          No response from the receiver in 2 days = your decision executes
          automatically.
        </p>
      </div>
    );
  ...
  const getReceiverActions = () => {
    ...
    if (
      receipt.receiver_decision === "accept" ||
      receipt.receiver_decision === "reject"
    )
      return null;
        ...
        <div className="bg-accent/10 border border-accent/20 rounded-xl p-4 flex gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Waiting for the sender to release funds. If they don't respond in
              2 days, the payment will be automatically released to you.
            </p>
          </div>
        </div>
      );
    ...
    return (
      <div className="space-y-3">
        ...
        <Button
          ...
          size="lg"
          className="w-full"
          onClick={() => startDecision("delivered")}
        >
      ...
      <div className="space-y-3">
        ...
        {receipt.sender_decision && (
          <div className="bg-secondary rounded-xl p-4 border border-border">
            ...
            <p className="font-semibold text-foreground mt-1">
              {labels[receipt.sender_decision] || receipt.sender_decision}
            </p>
        ...
        {receipt.receiver_decision && (
          <div className="bg-secondary rounded-xl p-4 border border-border">
            ...
            <p className="font-semibold text-foreground mt-1">
              {labels[receipt.receiver_decision] || receipt.receiver_decision}
            </p>
      ...
      <div className="pt-24 pb-16 px-4">
        <div className="container mx-auto max-w-lg">
          ...
          >
            ...
            <div className="bg-card rounded-2xl shadow-card overflow-hidden border border-border">
              <div className="bg-gradient-hero p-6 text-primary-foreground">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div
                    className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold ${statusCfg.bgClass}`}
                  >
                    {isSettling ? (
                ...
                {receipt.status === "pending" &&
                  ...
                  protectionFee > 0 && (
                    <p className="text-sm mt-1.5 opacity-80">
                      + {formatNaira(protectionFee)} protection fee ={" "}
                      <strong>
                        {formatNaira(receipt.amount + protectionFee)}
                      </strong>{" "}
                      total
                    </p>
                  )}
              ...
              <div className="p-6 space-y-5">
                ...
                {receipt.status === "active" && (
                  <div className="flex items-start gap-2.5 bg-primary/5 rounded-xl p-3">
                    ...
                    <p className="text-xs text-muted-foreground leading-relaxed">
                      ...
                      is held safely in Payscrow escrow until both parties
                      agree.
                    </p>
                  </div>
                )}
                ...
                {isSettling && (
                  <div className="flex items-start gap-2.5 bg-blue-500/5 rounded-xl p-3">
                    ...
                    <p className="text-xs text-muted-foreground leading-relaxed">
                      Settlement is being processed. Funds will arrive in bank
                      accounts shortly. This page will update automatically.
                    </p>
                  </div>
                )}
                        ...
                        <div className="bg-warning/10 border border-warning/20 rounded-xl p-4 space-y-3">
                          <div className="flex items-start gap-2">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground mt-1">
                                This receipt is complete. If funds are owed to
                                you, they cannot be sent without your bank
                                account details. Add them in Settings, then
                                contact support or the admin will process your
                                settlement shortly.
                              </p>
                            </div>
                          </div>
            ...
            {/* ── Auto-execute timer ───────────────────────────────────── */}
            {receipt.decision_auto_execute_at &&
              receipt.status === "active" && (
                <DisputeTimer
            ...
            {receipt.status === "unresolved" && (
              <div className="bg-destructive/10 border border-destructive/20 rounded-xl p-4 flex gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    The 4-day dispute window expired. An admin is reviewing the
                    evidence and will make a final settlement decision shortly.
                  </p>
                </div>
              </div>
            )}
                ...
                >
                  ...
                  {decisionType === "release_specific" && (
                    <div className="space-y-2">
                      ...
                      />
                      {decisionAmount &&
                        parseFloat(decisionAmount) < receipt.amount && (
                          <p className="text-xs text-muted-foreground">
                            Receiver gets{" "}
                            {formatNaira(parseFloat(decisionAmount))}, you get{" "}
                            {formatNaira(
                              receipt.amount - parseFloat(decisionAmount),
                            )}{" "}
                            back.
                          </p>
                        )}
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea
                      placeholder="Explain clearly so the other party understands..."
                  ...
                  <div className="flex gap-3 pt-1">
                    ...
                    <Button
                      ...
                      className="flex-1"
                      variant={
                        decisionType === "reject" ? "destructive" : "hero"
                      }
